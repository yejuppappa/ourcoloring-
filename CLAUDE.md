...
### 사진 변환 기능 (갤러리와 동등한 양대축)
- "내 아이 사진, 우리집 거실, 아이의 장난감" → 색칠공부 도안으로 변환
- 3가지 배경 모드 + 3단계 난이도
- 갤러리(도안 탐색)와 사진 변환(나만의 도안)이 서비스의 양대 기둥. 사이드바에서 동등하게 접근 가능.
...
| 네비게이션 | 좌측 사이드바 기반 | 상단 헤더만으로는 "도구 사이트" 느낌. 사이드바로 Freepik 같은 "플랫폼" 구조감 확보. 갤러리와 사진변환 양대축 동등 노출 |
...
| 2026-02-28 | **🔄 네비게이션 전면 변경: 상단 헤더 → 좌측 사이드바 기반 플랫폼 구조** |
| 2026-02-28 | 갤러리와 사진변환을 동등한 양대축으로 재배치 |
